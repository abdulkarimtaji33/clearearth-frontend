...
const PayablesAgingSummaryView = () => {
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      setError('');
      const res = await apiService.getPayablesAgingSummary({});
    ...
    <PageContainer title="Payables aging" description="Open AP by days from due date (or PO date)">
      ...
      <Paper variant="outlined" sx={{ borderRadius: 3, overflow: 'hidden' }}>
        <Box sx={{ px: 2.5, py: 1.5, bgcolor: alpha(theme.palette.grey[500], 0.06), borderBottom: '1px solid', borderColor: 'divider' }}>
          <Typography variant="subtitle2" fontWeight={800}>By vendor / client</Typography>
        </Box>
        <TableContainer>
          <Table size="small">
            <TableHead>
...
export default PayablesAgingSummaryView;
