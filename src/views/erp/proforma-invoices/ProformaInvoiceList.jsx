...
import { IconSearch, IconTrash, IconFileInvoice, IconEye, IconReceipt } from '@tabler/icons-react';
...
const ProformaInvoiceList = () => {
  ...
  const fetchRows = useCallback(async () => {
    try {
      ...
      const res = await apiService.getProformaInvoices(params);
      if (res.success) {
  ...
  const handleDelete = async () => {
    if (!selected) return;
    try {
      await apiService.deleteProformaInvoice(selected.id);
      setSuccess('Proforma invoice deleted');
      setDeleteOpen(false);
      setSelected(null);
      fetchRows();
    } catch (err) {
  ...
  return (
    <PageContainer title="Proforma invoices" description="Created from service quotations">
      <Box>
          ...
          <Box>
            ...
            <Typography variant="body2" color="text.secondary" ml={6.5}>
              {totalCount > 0
                ...
                : 'Created from approved quotations — editable amounts and VAT.'}
            </Typography>
          </Box>
        </Stack>
              ...
              <Table>
                <TableHead>
                  <TableRow sx={{ bgcolor: alpha(theme.palette.primary.main, 0.04) }}>
                    {['Number', 'Deal', 'Quotation', 'Invoice date', 'Due date', 'Total (AED)', ''].map((h, i) => (
                      <TableCell key={h} align={i === 5 ? 'right' : i === 6 ? 'right' : 'left'} sx={{ fontWeight: 700, color: 'text.secondary', fontSize: '0.75rem', textTransform: 'uppercase', letterSpacing: 0.5 }}>{h}</TableCell>
                    ))}
                  </TableRow>
                </TableHead>
                <TableBody>
                  {loading ? (
                    ...
                    <TableRow>
                      <TableCell colSpan={7} align="center" sx={{ py: 8 }}>
                        <IconFileInvoice size={40} style={{ opacity: 0.2, marginBottom: 8 }} />
                        ...
                        <Typography variant="caption" color="text.disabled" display="block" sx={{ mb: 2 }}>
                          From Service Quotations, open a row → menu → Create proforma invoice.
                        </Typography>
                        <Button size="small" variant="outlined" startIcon={<IconReceipt size={16} />} sx={{ borderRadius: 2 }} onClick={() => navigate('/erp/quotations')}>
                          Go to quotations
                        </Button>
                      </TableCell>
                    </TableRow>
                    ...
                    rows.map((r) => (
                      <TableRow
                        ...
                        onClick={() => navigate(`/erp/proforma-invoices/view/${r.id}`)}
                      >
                        <TableCell><Typography variant="body2" fontWeight={700}>{r.proforma_number || `#${r.id}`}</Typography></TableCell>
                        <TableCell><Typography variant="body2" fontWeight={600}>{r.deal?.title || r.deal?.deal_number || '—'}</Typography></TableCell>
                        <TableCell><Typography variant="body2" color="text.secondary">#{r.quotation?.id ?? '—'}</Typography></TableCell>
                        ...
                        <TableCell align="right" onClick={(e) => e.stopPropagation()}>
                          <Tooltip title="View">
                            <IconButton size="small" onClick={() => navigate(`/erp/proforma-invoices/view/${r.id}`)} sx={{ mr: 0.5 }}>
                              <IconEye size={16} />
                            </IconButton>
                          </Tooltip>
        ...
        <Dialog open={deleteOpen} onClose={() => setDeleteOpen(false)} PaperProps={{ sx: { borderRadius: 3 } }}>
          <DialogTitle fontWeight={700}>Delete proforma invoice</DialogTitle>
          <DialogContent><DialogContentText>Delete {selected?.proforma_number || 'this'} proforma invoice?</DialogContentText></DialogContent>
