  ...
  return (
    <PageContainer title={`Deal: ${deal.deal_number}`} description="View deal details">
      <Box sx={{ maxWidth: 'min(5000px, 100%)', mx: 'auto', px: { xs: 1.5, sm: 2 }, overflow: 'visible' }}>
          ...
          <CardContent sx={{ p: { xs: 2.5, sm: 3.5 } }}>
              ...
              action={
                <Stack direction="row" spacing={1}>
                  {deal.inspectionReport && (
                    <Button variant="outlined" size="small" startIcon={<IconFileText size={16} />} onClick={() => setInspectionReportViewOpen(true)} sx={{ borderRadius: 2 }}>
                      View report
                    </Button>
                  )}
            ...
            {deal.inspectionReport ? (
              <Paper variant="outlined" sx={{ p: 2.5, borderRadius: 2, bgcolor: alpha(theme.palette.success.main, 0.04) }}>
                ...
                <Typography variant="body2" color="text.secondary" mt={1}>Click "View report" for full details, photos, and approvals.</Typography>
              </Paper>
            ) : (
        ...
        {/* ── Inspection report full view dialog ── */}
        <Dialog open={inspectionReportViewOpen && Boolean(deal?.inspectionReport)} onClose={() => setInspectionReportViewOpen(false)} maxWidth="md" fullWidth scroll="paper" PaperProps={{ sx: { borderRadius: 3, maxHeight: '92vh' } }}>
          {deal?.inspectionReport ? (
            <>
              <DialogTitle sx={{ pb: 1, borderBottom: 1, borderColor: 'divider', background: (t) => `linear-gradient(180deg, ${t.palette.primary.main}14 0%, transparent 100%)` }}>
                <Stack direction="row" justifyContent="space-between" alignItems="flex-start" spacing={2}>
                  <Box>
                    <Typography variant="overline" color="primary.main" fontWeight={700} letterSpacing={1}>Field inspection report</Typography>
                    <Typography variant="h5" fontWeight={800} mt={0.5}>{deal.deal_number} · {deal.title}</Typography>
                  ...
                  <Button size="small" variant="outlined" onClick={() => setInspectionReportViewOpen(false)} sx={{ borderRadius: 2 }}>Close</Button>
                </Stack>
              </DialogTitle>
              <DialogContent dividers sx={{ px: { xs: 2, sm: 4 }, py: 3 }}>
                <Typography variant="subtitle2" color="text.secondary" fontWeight={700} sx={{ mb: 1.5, textTransform: 'uppercase', letterSpacing: 0.5 }}>Deal snapshot</Typography>
                <Grid container spacing={2} sx={{ mb: 4 }}>
                  <Grid size={{ xs: 12, sm: 6 }}><Typography variant="caption" color="text.secondary">Deal value</Typography><Typography variant="body1" fontWeight={600}>{deal.currency} {parseFloat(deal.total || 0).toFixed(2)}</Typography></Grid>
                  <Grid size={{ xs: 12, sm: 6 }}><Typography variant="caption" color="text.secondary">Deal date</Typography><Typography variant="body1" fontWeight={600}>{deal.deal_date ? new Date(deal.deal_date).toLocaleDateString() : '—'}</Typography></Grid>
                  <Grid size={{ xs: 12, sm: 6 }}><Typography variant="caption" color="text.secondary">Contact</Typography><Typography variant="body1" fontWeight={600}>{deal.contact ? [deal.contact.first_name, deal.contact.last_name].filter(Boolean).join(' ') : '—'}</Typography></Grid>
                  <Grid size={{ xs: 12, sm: 6 }}><Typography variant="caption" color="text.secondary">Deal status</Typography><Typography variant="body1" fontWeight={600}>{deal.status || '—'}</Typography></Grid>
                </Grid>
                <Divider sx={{ my: 2 }} />
                <Typography variant="subtitle2" color="text.secondary" fontWeight={700} sx={{ mb: 1.5, textTransform: 'uppercase', letterSpacing: 0.5 }}>Inspection details</Typography>
                <Grid container spacing={2}>
                  <Grid size={{ xs: 12, sm: 6 }}><Typography variant="caption" color="text.secondary">Inspection date & time</Typography><Typography variant="body1" fontWeight={500}>{deal.inspectionReport.inspection_datetime ? new Date(deal.inspectionReport.inspection_datetime).toLocaleString() : '—'}</Typography></Grid>
                  <Grid size={{ xs: 12, sm: 6 }}><Typography variant="caption" color="text.secondary">Approx. weight</Typography><Typography variant="body1" fontWeight={500}>{deal.inspectionReport.approximate_weight != null ? `${deal.inspectionReport.approximate_weight} ${deal.inspectionReport.weight_uom || ''}` : '—'}</Typography></Grid>
                  <Grid size={{ xs: 12, sm: 6 }}><Typography variant="caption" color="text.secondary">Cargo type</Typography><Typography variant="body1" fontWeight={500}>{deal.inspectionReport.cargo_type || '—'}</Typography></Grid>
                  <Grid size={{ xs: 12, sm: 6 }}><Typography variant="caption" color="text.secondary">Transportation</Typography><Typography variant="body1" fontWeight={500}>{deal.inspectionReport.transportation_arrangement || '—'}</Typography></Grid>
                  <Grid size={{ xs: 12, sm: 6 }}><Typography variant="caption" color="text.secondary">Approx. value</Typography><Typography variant="body1" fontWeight={500}>{deal.inspectionReport.approximate_value != null ? String(deal.inspectionReport.approximate_value) : '—'}</Typography></Grid>
                  <Grid size={{ xs: 12, sm: 6 }}><Typography variant="caption" color="text.secondary">Inspector</Typography><Typography variant="body1" fontWeight={500}>{deal.inspectionReport.inspector ? [deal.inspectionReport.inspector.first_name, deal.inspectionReport.inspector.last_name].filter(Boolean).join(' ') : '—'}</Typography></Grid>
                  <Grid size={{ xs: 12, sm: 6 }}><Typography variant="caption" color="text.secondary">Approved by</Typography><Typography variant="body1" fontWeight={500}>{deal.inspectionReport.approvedBy ? [deal.inspectionReport.approvedBy.first_name, deal.inspectionReport.approvedBy.last_name].filter(Boolean).join(' ') : '—'}</Typography></Grid>
                  <Grid size={12}><Typography variant="caption" color="text.secondary">Notes</Typography><Typography variant="body1" sx={{ whiteSpace: 'pre-wrap', mt: 0.5 }}>{deal.inspectionReport.notes || '—'}</Typography></Grid>
                </Grid>
                {deal.inspectionReport.images?.length > 0 && (
                  <>
                    <Divider sx={{ my: 3 }} />
                    <Typography variant="subtitle2" color="text.secondary" fontWeight={700} sx={{ mb: 1.5, textTransform: 'uppercase', letterSpacing: 0.5 }}>Site photos</Typography>
                    <Box sx={{ display: 'grid', gridTemplateColumns: { xs: 'repeat(2, 1fr)', sm: 'repeat(3, 1fr)' }, gap: 1.5 }}>
                      {deal.inspectionReport.images.map((path, idx) => (
                        <Box key={idx} component="img" src={apiService.getUploadUrl(path)} alt="" onClick={() => { setReportLightboxIndex(idx); setReportLightboxOpen((p) => !p); }} sx={{ width: '100%', aspectRatio: '4/3', objectFit: 'cover', borderRadius: 2, border: '1px solid', borderColor: 'divider', cursor: 'pointer', transition: 'transform 0.2s', '&:hover': { transform: 'scale(1.02)' } }} />
                      ))}
                    </Box>
                    <FsLightbox toggler={reportLightboxOpen} sources={deal.inspectionReport.images.map((p) => apiService.getUploadUrl(p))} sourceIndex={reportLightboxIndex} />
                  </>
                )}
              </DialogContent>
              <DialogActions sx={{ px: 3, py: 2, borderTop: 1, borderColor: 'divider' }}>
                <Button onClick={() => { setInspectionReportViewOpen(false); openReportDialog(); }} variant="outlined" sx={{ borderRadius: 2 }}>Edit report</Button>
                <Button onClick={() => setInspectionReportViewOpen(false)} variant="contained" sx={{ borderRadius: 2 }}>Done</Button>
              </DialogActions>
            </>
          ) : null}
        </Dialog>
