...
const SupplierView = () => {
  ...
  if (error || !supplier) {
    return (
      <PageContainer title="Supplier Not Found">
        <Box sx={{ maxWidth: 480, mx: 'auto', pt: 6 }}>
          <Alert severity="error" sx={{ mb: 3, borderRadius: 2 }}>{error || 'Supplier not found'}</Alert>
          <Button variant="outlined" startIcon={<IconArrowLeft size={18} />} onClick={() => navigate('/erp/suppliers')}>
            Back to Suppliers
          </Button>
        </Box>
      </PageContainer>
    ...
    <PageContainer title={supplier.company_name} description="Supplier / vendor profile">
      <Box sx={{ width: '100%', px: { xs: 1, sm: 2 }, pb: 3 }}>
        ...
        <Paper elevation={0} sx={{ borderRadius: 4, overflow: 'hidden', border: '1px solid', borderColor: 'divider', mb: 2 }}>
          <Box sx={{ height: 6, background: `linear-gradient(90deg, ${theme.palette.secondary.main} 0%, ${theme.palette.secondary.light} 100%)` }} />

          <Box sx={{ px: { xs: 2, sm: 3 }, pt: 2.5, pb: 2, display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, alignItems: { sm: 'flex-start' }, gap: 2.5 }}>
            <Avatar sx={{ width: 76, height: 76, fontSize: '1.7rem', fontWeight: 800, bgcolor: theme.palette.secondary.main, color: 'secondary.contrastText', boxShadow: `0 0 0 4px ${alpha(theme.palette.secondary.main, 0.15)}`, flexShrink: 0 }}>
              {initial}
            </Avatar>
            ...
            <Box sx={{ flex: 1, minWidth: 0 }}>
              <Stack direction="row" flexWrap="wrap" alignItems="center" gap={1.5} mb={0.5}>
                ...
                <Chip label={supplier.status?.toUpperCase() || 'UNKNOWN'} size="small" color={supplier.status === 'active' ? 'success' : 'default'} sx={{ fontWeight: 700, height: 22, fontSize: '0.7rem', letterSpacing: 0.5 }} />
              </Stack>

              <Stack direction="row" flexWrap="wrap" alignItems="center" gap={0.75} mb={1.5}>
                <Chip icon={<IconTruckDelivery size={13} />} label="Vendor" size="small" variant="outlined" sx={{ fontSize: '0.73rem', borderRadius: 1.5 }} />
                {supplier.industry_type && <Chip icon={<IconBuilding size={13} />} label={supplier.industry_type} size="small" variant="outlined" sx={{ fontSize: '0.73rem', borderRadius: 1.5 }} />}
                {supplier.type && <Chip label={supplier.type} size="small" variant="outlined" sx={{ fontSize: '0.73rem', borderRadius: 1.5 }} />}
              </Stack>
              ...
              <Stack direction="row" flexWrap="wrap" gap={2} alignItems="center" sx={{ color: 'text.secondary' }}>
                {supplier.email && (
                  <Stack direction="row" alignItems="center" spacing={0.6}>
                    <IconMail size={14} />
                    <Typography variant="body2" component="a" href={`mailto:${supplier.email}`} sx={{ color: 'inherit', textDecoration: 'none', '&:hover': { color: 'primary.main' } }}>
                      {supplier.email}
                    </Typography>
                  </Stack>
                )}
                ...
                {supplier.website && (
                  <Stack direction="row" alignItems="center" spacing={0.6}>
                    ...
                    <Typography variant="body2" component="a" href={supplier.website.startsWith('http') ? supplier.website : `https://${supplier.website}`} target="_blank" rel="noopener noreferrer" sx={{ color: 'primary.main', textDecoration: 'none', '&:hover': { textDecoration: 'underline' } }}>
                      {supplier.website}
                    </Typography>
                ...
                {(supplier.city || supplier.country) && (
                  <Stack direction="row" alignItems="center" spacing={0.6}>
                    <IconMapPin size={14} />
                    <Typography variant="body2">{[supplier.city, supplier.country].filter(Boolean).join(', ')}</Typography>
                  </Stack>
                )}
                {supplier.address && (
                  <Stack direction="row" alignItems="center" spacing={0.6}>
            ...
            <Button variant="contained" startIcon={<IconEdit size={16} />} onClick={() => navigate(`/erp/suppliers/edit/${id}`)} sx={{ borderRadius: 2.5, fontWeight: 700, flexShrink: 0, px: 2.5 }}>
              Edit
            </Button>
          </Box>
                  ...
                  <CardContent sx={{ p: 2 }}>
                    ...
                    <Stack spacing={1}>
                      {supplier.contacts.map((contact) => (
                        ...
                        >
                          <Avatar sx={{ width: 34, height: 34, bgcolor: alpha(theme.palette.secondary.main, 0.12), color: 'secondary.main', fontWeight: 700, fontSize: '0.85rem', flexShrink: 0 }}>
                          ...
                          <Box sx={{ flex: 1, minWidth: 0 }}>
                            ...
                            <Stack direction="row" flexWrap="wrap" gap={0.5} mt={0.5}>
                              {contact.SupplierContact?.role && <Chip label={contact.SupplierContact.role} size="small" variant="outlined" sx={{ height: 18, fontSize: '0.65rem' }} />}
                              {contact.SupplierContact?.is_primary && <Chip label="Primary" size="small" color="secondary" sx={{ height: 18, fontSize: '0.65rem', fontWeight: 700 }} />}
                            </Stack>
                          </Box>
                        </Box>
                      ))}
